 ...
 * GET /api/portfolio
 * 
 * Returns cached portfolio data for the authenticated user with automatic cache busting:
...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.info(`Fetching portfolio data for user ${user.id} from cache...`)
    ...
    logger.info(`Portfolio data fetched for user ${user.id} in ${duration}ms`)
