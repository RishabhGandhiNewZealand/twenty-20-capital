...
export async function GET(request: NextRequest) {
  try {
    ...
    const holdingsBySymbol = new Map<
      string,
      { shares: number; totalCostNZD: number; name: string; currency: string }
    >()
    let sp500Shares = 0
    let currentCostBasis = 0
    let soldCapitalAvailable = 0

    const tradeDates = [...new Set(sortedTrades.filter(t => t.type === 'Buy').map(t => t.date))]
    ...
    tickers.forEach((ticker, index) => {
      priceMap.set(ticker, tickerPrices[index])
    })
    ...
    holdingsBySymbol.forEach((holding, symbol) => {
      ...
      const currentValueNZD =
        holding.shares * currentPrice * (holding.currency === 'USD' ? currentExchangeRate : 1)
    ...
    const cashPositionNZD = soldCapitalAvailable
    if (cashPositionNZD > 0.01) {
      holdings.push({
        ...
        allocation: 0,
        currency: 'NZD',
        avgPriceInstrument: 1,
        isCash: true,
      })
      totalValueNZD += cashPositionNZD
    }

    holdings.forEach(h => {
      h.allocation = totalValueNZD > 0 ? (h.currentValueNZD / totalValueNZD) * 100 : 0
    })
    holdings.sort((a, b) => b.allocation - a.allocation)

    // Compute avg price per instrument currency via calculatePortfolioData
    const { holdings: baseHoldings, exitedPositions } = calculatePortfolioData(sortedTrades)
    const avgMap = new Map<string, { avgPriceNZD: number; avgPriceUSD?: number }>()
    baseHoldings.forEach(h => {
      avgMap.set(h.symbol, { avgPriceNZD: h.avgPriceNZD, avgPriceUSD: h.avgPriceUSD })
    })
    holdings.forEach(h => {
      if (h.isCash) return
