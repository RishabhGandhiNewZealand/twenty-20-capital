...
import { getTradeValueNZD, sortTradesInLedgerOrder } from './portfolio-cash'
...
export function calculateDailyReturns(
  ...
  const cashBalanceByDate = new Map<string, number>()
  ...
  sortedTrades.forEach(trade => {
    ...
    cashBalanceByDate.set(dateStr, runningSoldCapital)
  ...
  while (processDate <= endDate) {
    ...
    if (cashBalanceByDate.has(dateStr)) {
      lastCashBalance = cashBalanceByDate.get(dateStr)!
    }
    ...
    // Cash remains inside the portfolio until it is used for a later buy.
    let portfolioValue = lastCashBalance
