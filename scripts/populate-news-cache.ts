...
// Main function to populate the cache
async function populateNewsCache() {
  try {
    logger.info('Starting news cache population...')
    ...
    for (let i = 0; i < companies.length; i++) {
      ...
      try {
        ...
        if (shouldCache) {
          ...
          successCount++ // Still count as success since we processed it
        }
    ...
    logger.error('Fatal error in cache population:', error)
    process.exit(1)
  }
}
